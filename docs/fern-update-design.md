...
- 读完设置就检查一次，之后每 6 小时一次。**这里原本写的是「延迟 30 秒」**，
  改掉的理由是那三十秒买不到什么——一份几 KB 的清单跟补全抢不到带宽，而代价是
  打开就关掉的人永远查不到更新。要等的只是读盘：`automatic` 关着的时候一个请求
  都不该发，而那个取值在读盘之前不知道。
- 检查失败按 2 分钟起退避重试，上限 30 分钟。开机那一次赶上网卡还没连上是常态，
  而没有这一条的话下一次要等六小时——那六小时里界面什么都不会说，因为检查
  失败本来就是沉默的（§5.7）。
- 检查到新版本**不弹窗**，只在设置入口放一个不打扰的标记。
- 下载在后台静默进行，装完了才在设置里说「重启后生效」，而不是「立即重启」。
  后台这一次失败也不出声，下一轮检查会再来——出声的只有用户自己按下按钮的那一次。
- 有游戏在运行时不下载（带宽和磁盘这时候属于游戏），也不给重启按钮。
- 用户能关掉自动更新（设置里 `about` 段），关掉之后**连检查都不做**。这一条与
  本节早先的写法（「仍然检查、只是不下载」）相反：那个开关在界面上的说明是
  「关闭后不会发出任何检查更新的请求」，而一个说了不联网却仍在联网的开关，
  比没有这个开关更糟。
- **验签不提供开关。**
