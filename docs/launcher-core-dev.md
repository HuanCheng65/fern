...
- tokio + `Semaphore` 控制并发（64 左右起步）。全进程一个客户端，各调用点从它分支出去，所以那个数是整台机器上真正同时开着的连接数。
- **对账在下载那道闸的外面。** 整批先按 256 个一组交给阻塞线程判断「磁盘上那份算不算数」，命中的当场结账，只有真要下的才进下载路径。磁盘的活和网络的活不抢同一份配额——一个已经装好的实例，四千次 stat 没有理由排在一道为网络设的队列后面。
- 每个任务：直接 GET → 写临时文件（带缓冲，不是每个 chunk 一次系统调用）→ 校验 sha1 → rename 到位。rename 是先试、失败了再删了重来，而不是每个文件先 stat 一次问「目的地在不在」。校验失败换源重试，重试耗尽则记入失败清单。
- **大文件切成几段并发拉**（8 MB 以上，服务器认 Range 才走）。模型抄 aria2 的两层：**片**是 1 MB 的固定网格，切点全落在网格上，所以「下到哪了」是一张位图，位图落盘就是跨进程续传；**段**是临时分配给一条连接的活，可以被抢。一条工人干完手上的活，就去剩余量最大的在途段里抢后半截——静态均分解决不了长尾，最慢的那条连接会定义整个文件的完成时间。
  - **段从全局那道闸里取名额，取不到就不开**，而且要**反复取**：一批文件刚开工时名额是满的，等小文件陆续下完才腾得出来，而那正是大文件还在跑的时候。只在开头试一次等于永远招不到人。分段因此是空闲产能的用途，不是新增产能。
  - **段数不能超过段的个数。** 多出来的工人无活可领，只能去抢，而抢一次就意味着苦主那条连接上有一批已经在路上的字节要丢掉（实测本机回环上，一个 12 MB 的文件能因此多收 7 MB）。抢是抹平长尾的手段，不是分活的常态。
  - 段也不能切太碎：每个 Range 请求都是一条新的 HTTP/2 流，流的初始窗口要几个来回才涨上去，切碎了就把这笔爬坡钱反复付。下限 4 MB。
  - 段是乱序落盘的，所以 sha1 只能等字节齐了整体读一遍，不能就着流现算。
  - **一片记成「下完了」之前必须真的落盘。** 位图说下完了、字节还在内存缓冲里，正是续传拼出一份坏文件的方式。
  - 收益取决于线路：瓶颈在每连接限速时收益很大（实测一条连接 1.97 MB/s、64 条聚合 8.0 MB/s），瓶颈在上游或本机出口时一分收益也没有（实测裸单流和裸 8 段并发都是 22 秒）。没收益的时候它退化成单流，代价是 1–3%。
- 单流那条路仍然支持断点续传（Range），大文件断在半路时不重下已经落盘的部分。
- **HTTP/2 必须开着**（reqwest 的 `http2` feature，加自适应窗口）。关掉的话「同时下 64 个」在网络上的样子是同时开 64 条 TCP、握 64 次 TLS，而补全一个实例要下四千个文件。窗口不开自适应则相反：h2 的 64 KB 初始窗口在高延迟线路上比 HTTP/1.1 还慢。
- **上游给了压缩变体就走压缩。** Mojang 的运行时清单给多数文件配了 lzma 变体，实测 java-runtime-gamma（linux-x64）从 95.2 MB 降到 68.2 MB。省不到更多是因为最大的 `lib/modules`（55.5 MB）恰恰没有变体——它本身就是压好的 jimage；给了变体的那些省得很多（`libjvm.so` 23.1 MB → 5.3 MB）。所以这里必须按文件判断有没有，而不是假定都有。任务描述的始终是成品（`path`/`sha1`/`size`），压缩只是运输方式：落盘前解开，成品的 sha1 照验不误，进度按网上真跑的字节算（否则分子按压缩后计、分母按解压后计，进度条永远到不了头）。
- 每批下完往 `fern.log` 写一行账：跳过/下载/失败各几个、对账和传输各花多久、发了多少请求其中多少是重试。「下载很慢」是最难查的那类报告，没有这一行就只能靠猜。
...
版本名是人起的，`version::is_safe_id` 因此只挡一件事：跳出 `versions/` 目录。判据是「它是不是一个普通的路径分量」，不是字符白名单——`Simply Craftmine` 带空格，整合包常常直接叫中文名，白名单会把它们一律判成非法（扫不出版本，或者添加之后一启动就报「版本 id 无法作为目录名」）。跳过的目录连同原因一并返回（`ExternalScan::skipped`），否则「一个都没扫出来」这件事无从解释。
...
**崩溃分析回答「刚才为什么死了」，预检查回答「按现在这样点下去会不会死」。** 两者共用同一个动作枚举（`launch::crash::Action`），于是界面上那颗「去安装 / 删除」按钮是同一颗。

崩溃分析是三步纯函数流水线，各自能单独测：
...
- **证据不是一份文本**：`crash-reports/*.txt` 最完整；游戏来不及写时只有控制台；原生崩溃时两者皆空，全部信息在 `hs_err_pid*.log` 里，而它的 `# Problematic frame:` 下一行直接写着崩在哪个原生库（`nvoglv64.dll` 就是 N 卡驱动）。
- **解析只提取不判断**，而且只有两个通用原语：`Sections`（按 `-- 段名 --` 切）和 `Indented`（按缩进读成树）。崩溃报告是分段文档加缩进树，用一堆正则去啃它是把简单的事做难。每个字段都是 `Option`，解析失败退回全文匹配，新的一层最坏等于旧的那一层。
- **规则表**（`rules/crash.toml`）：一条规则 = 一个必须命中的正则 + 若干只做收窄的守卫（scope / loader / minecraft 区间）。取值只靠**命名捕获组**。不是首个命中获胜，而是排序：`exact`（认得出具体是哪两个东西撞了）> `named`（说得出主语）> `generic`（只认类别）。
- **归因独立于规则**：一条规则都没命中时，「崩在 Sodium 0.6.0 的代码里」往往是唯一线索。判据是包名前缀——原版类名会随版本混淆，但模组自己的包名不会。mixin 失败特判：消息里直接写着 `sodium.mixins.json`，比翻栈更准。

预检查（`launch::preflight`）读 mods 目录里那些 jar 的元数据，不联网：缺前置、同一 modid 装了两份、模组不是给这个加载器的、声明的游戏版本对不上、前置被禁用了。**只报确定的事**——版本区间看不懂就当满足（`launch::ranges`），可选依赖缺了不报。它**不阻止启动**。

三条测试守着规则表：每条规则必须有一份 fixture 且必须命中它、没有孤儿 fixture、以及**一份正常退出的日志不许命中任何规则**（规则表最常见的腐化方式是某条正则写太宽）。

语料不进仓库：`.github/collect-crashes.py` 按 MC 版本分档抓取并脱敏（**日志里可能带 access token**），`cargo run -p fern-core --example crash_coverage -- corpus/` 报覆盖率，并把没命中的按根因异常聚类——那就是下一条规则该写什么。
