...
扫描时**不能**套用 `version::is_safe_id`——那条规则说的是我们自己下载的版本该叫什么（只放行 ASCII）。别人目录里的版本名是人起的：`1.20.1-Fabric 0.16.9` 带空格，整合包常常直接叫中文名。按那条规则扫，一个装满版本的目录会扫出一片空白。要求只有一条：名字必须是一个普通的路径分量，不能借 `..` 或分隔符跳出去。跳过的目录连同原因一并返回（`ExternalScan::skipped`），否则「一个都没扫出来」这件事无从解释。
