...
**实例详情页 — 分配预览。** 结论是一根尺，不是三行字：分配、上限、物理内存本来就是三个嵌套的区间，画出来一眼就完。

```
内存                                              8 GB 自动
        ▲上次峰值 6.3 GB
├──────█████████████──────────┤·····················┤
基于 186 个 Mod、光影、16 区块渲染距离
水位健康，维持 8 GB              上限 16 GB／本机 32 GB
```

三条要点：

- **只读和可拖是同一根尺。** 自动那一档不给拖动手柄（控件退后），按「手动指定」之后同一根尺变得可拖，几何一个像素不动。上一版自动是一段文字、手动是一根滑杆，切换时控件换人、高度跟着跳。
- **幽灵刻度是这根尺的价值所在**：自动会给多少、上次实际用到多少，画在你正要拖的那条线上。为此 `InstanceRuntime` 多了一个 `measured`（`MemoryHistory`：会话数、上次峰值、live set、那句结论）——`explanation` 里那句「上次峰值 6.3 GB」是给人读的**句子**，画刻度要的是**数**，从中文句子里正则抠数字等于把文案的措辞变成接口。没有足够历史就是 `None`，那时不画这道刻度。
- **`explanation` 按 topic 各自落位**，不再串成一长条：`basis` 在尺下面，`limit` 只在真的咬住时出现，`history` 那一类不出现在界面上——它的数字变成了刻度，结论由 `measured.note` 给，一件事只说一遍。

`source == UserJvmArgs` 时整节塌成一句话（「堆大小由额外 JVM 参数中的 -Xmx 决定」）加一个跳到那一行的入口：那时后端报的 `xmx_mb` 是 0，画一根推不动的尺、或者照着显示「自动 · 0 GB」，两种都是撒谎。

**设置页的全局上限用同一个组件**，只是拖的就是上限本身、右端是物理内存，所以不画那堵墙。两屏说同一种视觉语言。
