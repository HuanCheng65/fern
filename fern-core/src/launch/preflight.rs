...
/// 无论如何都在的那几个：游戏、Java、加载器自己。
///
/// **这几条不会过期，所以它们可以写死。** 每个加载器都把游戏和 JVM 注册成内置
/// 模组，而 `fabricloader` / `quilt_loader` 是加载器给自己起的名字——这是一个不
/// 变的对应关系，不是一份会随版本增删的清单。真会变的那部分（加载器还捎带了
/// 什么）由 [`bundled`] 去读，两者不是一回事。
///
/// 它同时兜住「那个 jar 还不在磁盘上」：新建或刚接手、还没补全过的实例，
/// libraries 里空空如也，[`bundled`] 两手空空地回来。那时如果连 `fabricloader`
/// 都要报一条缺前置，说的既不是事实，也不是用户此刻该处理的事。
///
