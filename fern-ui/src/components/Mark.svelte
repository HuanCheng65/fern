<script lang="ts">
  ...
  let { size = 20, spinning = false, progress, pad, label }: Props = $props()
  ...
  const cycle = $derived(1800 * (theme.motionScale || 1))
...
<svg
  ...
  class:spin={spinning && theme.motionScale > 0}
...
<style>
  ...
  /* 系统级的减弱动效也要认。设置里的档位由 theme.motionScale 管，这条管系统。 */
