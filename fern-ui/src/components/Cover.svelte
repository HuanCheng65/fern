<script lang="ts">
   ...
   * 和构图，当前小时决定色温。所以列表里每一行的色块不是随便挑的四个颜色，
   * 而是这个实例的脸——同一个实例在哪里出现都长一样，换个实例就是另一张画。
   *
   * 尺寸从 CSS 拿（元素多大就画多大），所以同一个组件能当 32px 的行内缩略图，
   * 也能当详情页顶上的整条封面。
   */
  ...
  let { seed, quality = 0.55, class: className = '' }: Props = $props()
  ...
  async function render() {
    ...
    const mine = ++token
    const options = { name: seed, hours: 0 }
    if (supportsBiomeWorker) {
      try {
        const bitmap = await renderBiome(w, h, options, 0, quality).promise
